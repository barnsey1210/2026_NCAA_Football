...
def standardized_spread_hold_pct(home_line, home_price, away_line, away_price):
    """
    Calculate sportsbook hold from the best available home-side and away-side prices,
    adjusted for spread gap/overlap.

    home_line is from home-team perspective.
      Example: North Dakota State -9.5 => home_line = -9.5

    away_line is from away-team perspective.
      Example: Jacksonville State +10 => away_line = +10

    Raw hold:
      implied(home_price) + implied(away_price) - 100

    If home_line + away_line > 0, the bettor has a gap/middle cushion.
      Example: -9.5 + +10 = +0.5
      That should REDUCE hold.

    If home_line + away_line < 0, there is an overlap/dead-zone cost.
      Example: -10.5 + +10 = -0.5
      That should INCREASE hold.

    We still display only "Hold +/-x.x%" and do not show middle language.
    """
    try:
