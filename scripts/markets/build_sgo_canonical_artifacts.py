...
def main():
 ...
 selected=manifest.get("resolved_canonical_week")

 expected_ids={
  str(x) for x in manifest.get("expected_canonical_game_ids",[])
  if str(x)
 }
 manifest_mapped_ids={
  str(x) for x in manifest.get("mapped_canonical_game_ids",[])
  if str(x)
 }
 accepted_ids={
  str(q["canonical_game_id"]) for q in quotes
  if q.get("canonical_game_id")
 }

 if not expected_ids:
  expected_ids=set(manifest_mapped_ids or accepted_ids)

 missing_mapping_ids=sorted(expected_ids-manifest_mapped_ids)
 unavailable_ids=sorted(manifest_mapped_ids-accepted_ids)

 obs_by_game=defaultdict(list)
 for row in obs:
  gid=str(row.get("canonical_game_id") or "")
  if gid:
   obs_by_game[gid].append(row)

 unavailable_reasons={}
 for gid in unavailable_ids:
  rows=obs_by_game.get(gid,[])
  if not rows:
   unavailable_reasons[gid]="no_supported_quote_observations"
  elif all(truth(row.get("stale")) for row in rows):
   unavailable_reasons[gid]="stale_quotes_only"
  elif all(not truth(row.get("available")) for row in rows):
   unavailable_reasons[gid]="unavailable_quotes_only"
  elif all(truth(row.get("suspended")) for row in rows):
   unavailable_reasons[gid]="suspended_quotes_only"
  else:
   unavailable_reasons[gid]="no_valid_paired_market"

 weeks=sorted({
  int(q.get("canonical_site_week"))
  for q in quotes
  if str(q.get("canonical_site_week","")).strip() not in {"","None"}
 })
 ...
 for week in weeks:
  ids={
   str(q["canonical_game_id"]) for q in quotes
   if str(q.get("canonical_site_week"))==str(week)
  }
  by_week[str(week)]={
   "accepted_games":len(ids),
   "game_ids":sorted(ids),
   "quote_rows":sum(
    1 for q in quotes
    if str(q.get("canonical_site_week"))==str(week)
 ...
 acceptance_ok=(
  global_ok
  and not missing_mapping_ids
  and not manifest.get("next_cursor_present",False)
 )

 block_reasons=[]
 if manifest.get("coverage_status")!="COMPLETE":
  block_reasons.append("provider coverage incomplete")
 if manifest.get("next_cursor_present",False):
  block_reasons.append("pagination cursor remains")
 if manifest.get("events_ambiguous",0):
  block_reasons.append("ambiguous event mapping")
 if missing_mapping_ids:
  block_reasons.append("expected canonical games not mapped")

 coverage={
  "schema_version":"canonical-market-coverage-v2",
  "provider":"sports_game_odds",
  "scope":"all upcoming mapped canonical games",
  "selected_week":selected,
  "selected_week_is_display_only":True,
  "canonical_weeks_covered":weeks,
  "coverage_by_week":by_week,

  "expected_canonical_games":len(expected_ids),
  "expected_canonical_game_ids":sorted(expected_ids),

  "mapped_canonical_games":len(manifest_mapped_ids),
  "mapped_canonical_game_ids":sorted(manifest_mapped_ids),

  "missing_canonical_games":len(missing_mapping_ids),
  "missing_canonical_game_ids":missing_mapping_ids,

  "accepted_quote_games":len(accepted_ids),
  "accepted_quote_game_ids":sorted(accepted_ids),

  "mapped_games_without_accepted_quotes":len(unavailable_ids),
  "mapped_games_without_accepted_quote_ids":unavailable_ids,
  "mapped_games_without_accepted_quote_reasons":unavailable_reasons,

  "upcoming_schedule_games":manifest.get(
   "upcoming_schedule_games",
   manifest.get("upcoming_schedule_fbs_games"),
  ),
  "upcoming_schedule_without_sgo_event":manifest.get(
   "upcoming_schedule_without_sgo_event"
  ),
  "upcoming_schedule_without_sgo_event_ids":manifest.get(
   "upcoming_schedule_without_sgo_event_ids",[]
  ),

  "raw_pages_fetched":manifest.get("page_count",1),
  "next_cursor_remaining":manifest.get("next_cursor_present",False),
  "coverage_status":manifest.get("coverage_status"),
  "request_unit_estimate":manifest.get("estimated_api_cost",0),
  "request_unit_actual":manifest.get("actual_api_cost"),

  "acceptance_eligibility":acceptance_ok,
  "acceptance_block_reason":"; ".join(block_reasons),

  "unmatched_events":manifest.get("events_unmatched",0),
  "ambiguous_events":manifest.get("events_ambiguous",0),
  "accepted_quote_rows":len(quotes),
