...
(function(){
  ...
  function simRows(){
    return (DB.teams || []).map(t => {
      ...
      return {
        ...
        bowl: probNumber(t.bowl_eligibility_pct),
  ...
  function sortVal(r, key){
    ...
    if (key === 'win_edge') return n(r.win_market.edge) ?? -999;
  ...
  function simBoardTable(){
    ...
    const body = rows.map(r => {
      ...
      return `<tr>
        ...
        <td class="${n(r.win_market.edge) > 0 ? 'pos' : n(r.win_market.edge) < 0 ? 'neg' : ''}">${fmt(r.win_market.edge,2)}</td>
        <td>${confMarketTxt}</td>
