...
def normalize(rows,model_id,market,legacy=False):
 out=[]
 for r in rows:
  out.append({'model_id':model_id,'model_version':'v1','market_type':market,'checkpoint':r['checkpoint'],'threshold':float(r['threshold']),'n':int(r['n']),'record':r['record'],'win_pct':r.get('win_pct'),'roi':r.get('roi'),'beat_close_pct':r.get('beat_close_pct'),'won_line_move_pct':r.get('won_line_move_pct'),'avg_clv':r.get('avg_clv'),'median_clv':r.get('median_clv'),'positive_clv_pct':r.get('positive_clv_pct'),'clv_implied_ev':r.get('clv_implied_ev'),'avg_edge':r.get('avg_edge'),'median_edge':r.get('median_edge'),'sample_classification':r.get('sample_classification') or ('LEGACY_COMMON_SAMPLE' if legacy else 'RETROSPECTIVE_TIMING_UNVERIFIED')})
 return out
def main():
 ...
 decay=pd.read_csv(EARLY/'spread_edge_decay.csv');decay=decay[decay.model_id.eq('spread_4src_25_25_25_25_v1')];sd=[]
 for (threshold,origin,later),a in decay.groupby(['threshold','origin_checkpoint','later_checkpoint']):
  sd.append({'model_id':'standard_spread_4src_equal_v1','model_version':'v1','market_type':'spread','threshold':float(threshold),'origin_checkpoint':origin,'checkpoint':later,'n':len(a),'avg_origin_edge':a.origin_edge.mean(),'mean_remaining_edge':a.later_edge.mean(),'remaining_edge_pct':a.remaining_edge_pct.mean(),'edge_persistence_pct':a.threshold_persisted.mean(),'same_side_persistence_pct':a.same_side.mean(),'strengthened_pct':a.strengthened.mean(),'avg_clv':a.clv.mean(),'mode':'MATCHED_ORIGIN_COHORT'})
 td=[]
 ...
 payload={'schema_version':'historical-betting-analytics-v2','generated_at':datetime.now(timezone.utc).isoformat(),'season_range':'2021-2025','default_selection':{'spread':{'model_id':'standard_spread_4src_equal_v1','checkpoint':'SUN_9AM_ET','threshold':3.0},'total':{'model_id':'total_sp50_massey50_v1','checkpoint':'SUN_9AM_ET','threshold':3.0}},'metric_definitions':{'win_pct':'ATS for spread; O/U win percentage for total','roi':'flat realized return using preserved study pricing','beat_close_pct':'positive point CLV including unchanged closes in denominator','won_line_move_pct':'positive CLV among nonzero moves','clv_implied_ev':'null unless a validated market-specific conversion exists','independent_checkpoint':'each checkpoint uses its available sample','matched_decay':'same origin signal followed through later checkpoints'},'checkpoint_order':ORDER,'thresholds':TH,'models':models,'independent_checkpoint_performance':spread+totals,'matched_signal_decay':sd+td,'coverage':{'spread':'No input-complete Saturday 11 PM Standard cohort; Friday 2021 absent; checkpoint samples differ.','total':'Only preserved Sunday and close checkpoints; no Monday-Friday Total history.','dratings_total':'Prospective 2026 identity; historical cohort too limited for equivalent validation.'},'source_references':[str((EARLY/'spread_checkpoint_results.csv').relative_to(ROOT)),str((EARLY/'spread_edge_decay.csv').relative_to(ROOT)),str((EARLY/'total_checkpoint_results.csv').relative_to(ROOT)),str((COMP/'spread_threshold_checkpoint_summary.csv').relative_to(ROOT)),str(TOTAL.relative_to(ROOT))]};atomic(payload);print(f'Wrote {OUT}')
