...
const CONTROL_ACTIONS = new Map([
  ['/war-room/market','market'],
  ['/war-room/ratings','ratings'],
  ['/war-room/postgame','postgame']
]);
const RELAY_REQUESTS = new Map();
let CONTROL_WINDOW = null;
let OPERATOR_MODE = null;
...
function requestViaRelay(path, button, old){
  const action=CONTROL_ACTIONS.get(path);
  if(!action || !CONTROL_WINDOW || CONTROL_WINDOW.closed) throw new Error('Operator session is not connected');
...
async function requestOperation(path, button, runningLabel){
  ...
  if(OPERATOR_MODE==='relay'){
    try{requestViaRelay(path,button,old)}catch(err){
  ...
  try{
    ...
    if(response.status !== 202 || !payload.task_id) throw new Error('Missing 202 task acknowledgement');
    button.textContent = '✓ REQUESTED';
    status.textContent = `Task ${payload.task_id} · ${payload.status || 'REQUESTED'}`;
    await pollOperation(payload.task_id, status);
  }catch(err){
...
async function pollOperation(taskId, status){
  const terminal=new Set(['COMPLETED','COMPLETED_WITH_WARNINGS','FAILED','BLOCKED_BY_OVERLAP','DEFERRED_BY_DAILY_BACKBONE']);
  for(let attempt=0;attempt<240;attempt++){
    const response=await fetch(`${CONTROL_BASE_URL}/war-room/task/${encodeURIComponent(taskId)}`,{cache:'no-store',credentials:'include'});
    const payload=await response.json().catch(()=>({}));
    if(!response.ok) throw new Error(payload?.detail || `Task HTTP ${response.status}`);
    const task=payload.task || {};
    status.textContent=operationDetail(task);
    if(terminal.has(task.status)){
      if(task.status==='COMPLETED' || task.status==='COMPLETED_WITH_WARNINGS') await loadData();
      return task;
    }
...
async function detectOperator(){
  ...
  try{
    ...
    status.textContent='Authentication unavailable · connect operator session';
  }
}
