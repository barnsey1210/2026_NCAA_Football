...
async function detectOperator(){
  ...
  try{
    ...
    document.querySelectorAll('.operator-control').forEach(el=>el.disabled=false);
    status.textContent='Operator ready';
  }catch(err){
    document.querySelectorAll('.operator-control').forEach(el=>el.disabled=true);
    status.textContent=`Authentication unavailable · ${err.message}`;
  }
}
