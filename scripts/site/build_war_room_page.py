...
@media(max-width:900px){
  ...
  .mobile-edge-value.mobile-edge-yellow{color:var(--yellow)}
  .mobile-edge-value.mobile-edge-red{color:var(--red)}
...
function mobileEdgeMagnitudeClass(edge){
  const n=numericSortValue(edge);
  if(n===null) return 'mobile-edge-unavailable';
  if(n>3) return 'mobile-edge-green';
  if(n>=2) return 'mobile-edge-yellow';
  return 'mobile-edge-red';
}

function syncMobileHeaderOffset(){
  const controls=document.getElementById('mobileStickyBar');
  if(!controls) return;
