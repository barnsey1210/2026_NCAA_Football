...
function requestViaRelay(action, button, old, payload=null){
...
function requestOperation(action, button, runningLabel, payload=null){
  const old = button.textContent;
  button.disabled = true;
  button.textContent = runningLabel;
  const status = document.getElementById('operatorStatus');
  status.textContent = 'Submitting authenticated request…';
  try{requestViaRelay(action,button,old,payload)}catch(err){
