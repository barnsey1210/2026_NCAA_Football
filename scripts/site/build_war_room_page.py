...
.coach-head{display:flex;align-items:flex-start;justify-content:space-between;gap:5px;margin-bottom:4px}
...
.snapshot-opener-box{
  ...
  background:rgba(35,91,122,.22);
  border-radius:4px;
}
.snapshot-market-section{
  padding:6px 0 8px;
  border-bottom:1px solid rgba(104,132,157,.22);
}
.snapshot-market-section-title{
  color:#59dfff;
  font-size:9px;
  font-weight:950;
  letter-spacing:.08em;
...
.snapshot-open-row+.snapshot-open-row{
  border-top:1px solid rgba(115,174,204,.16);
  padding-top:6px;
}
.snapshot-open-kind,
...
.snapshot-book-row{
  padding:5px 0 6px;
  border-bottom:1px solid rgba(104,132,157,.13);
}
.snapshot-book-row:last-child{
  border-bottom:0;
}
.snapshot-book-top{
  display:flex;
...
.snapshot-latest-move-card{
  margin:0 0 7px;
  padding:7px 8px;
  border:1px solid rgba(255,190,75,.38);
  background:rgba(128,86,24,.18);
  border-radius:4px;
}
.snapshot-latest-move-head{
  display:flex;
  justify-content:space-between;
  align-items:center;
  gap:8px;
...
.snapshot-latest-move-head span{
  color:#ffd166;
  font-size:9px;
  font-weight:950;
  letter-spacing:.07em;
}
.snapshot-latest-move-head time{
  color:#aab7c3;
  font-size:8px;
  font-weight:700;
}
.snapshot-latest-move-meta{
  color:#66dcff;
  font-size:8px;
  font-weight:900;
  margin-bottom:2px;
}
.snapshot-latest-move-value{
  color:#ffffff;
  font-size:10px;
  font-weight:950;
  line-height:1.25;
...
.snapshot-market-block{
  padding:6px 0 7px;
  border-bottom:1px solid rgba(104,132,157,.22);
}
.snapshot-market-block:last-child{
  border-bottom:0;
}
...
.best-col:hover,
.best-col:focus-within{
  position:relative;
  z-index:250!important;
}
.best-col:hover .model-tooltip,
.best-col:focus-within .model-tooltip{
  position:relative;
  z-index:260!important;
}
.best-col .model-tooltip-panel{
...
.best-latest-move{
  ...
  margin-top:6px;
  padding-top:6px;
  border-top:1px solid rgba(105,145,173,.28);
}
.best-latest-move-label{
  color:#59dfff;
  font-size:8px;
  font-weight:950;
  letter-spacing:.08em;
}
.best-latest-move-book{
  color:#dce8f0;
  font-size:8px;
  font-weight:850;
}
.best-latest-move-value{
  color:#ffffff;
  font-size:9px;
  font-weight:950;
  white-space:nowrap;
}
.best-latest-move-time{
  color:#8da3b4;
  font-size:8px;
  font-weight:700;
}
...
/* Narrow-desktop spread projection: stack favorite logo above value
   so the projection cannot bleed into SPREAD SHADOW. */
...
@media(max-width:900px){
  ...
  .mobile-command-head{position:sticky;top:91px;z-index:24;border:1px solid var(--line2);border-radius:6px 6px 0 0;background:#102332;box-shadow:0 3px 8px rgba(0,0,0,.32)}
  ...
  .mobile-team-search::placeholder{color:#70859a}
  .mobile-team-search:focus{
    border-color:var(--cyan);
    box-shadow:0 0 0 1px rgba(69,217,237,.22);
  }
...
<div class="wr-shell">
  ...
  <section class="health-strip ratings-health-strip" id="ratingsHealthStrip">
    ...
    <div class="ratings-model-mode">
      <div class="ratings-model-notes">
        <div class="ratings-model-note">
          <span class="ratings-model-note-label">BEST MOVE ALERT</span>
          <span>Red Spread/Total Best = change detected within last 30 min</span>
        </div>
        <div class="ratings-model-note">
...
function currentRows(){
  ...
  return [...rows].sort((a,b)=>{
    ...
    if(SORT_KEY === 'date'){
      ...
      av = numericSortValue(
        displayedEdge(a,'spread')?.best_edge
      );

      bv = numericSortValue(
        displayedEdge(b,'spread')?.best_edge
      );
    }

    else if(SORT_KEY === 'total_edge'){
      av = numericSortValue(
        displayedEdge(a,'total')?.best_edge
      );

      bv = numericSortValue(
        displayedEdge(b,'total')?.best_edge
      );
    }
...
function renderSummary(){
  ...
  const spreadEdges = rows
    .map(g => numericSortValue(displayedEdge(g,'spread')?.best_edge))
    .filter(v => v !== null);

  const totalEdges = rows
    .map(g => numericSortValue(displayedEdge(g,'total')?.best_edge))
    .filter(v => v !== null);
...
function manualDisplayActive(game){
  return (
    String(game?.operator_model?.mode || 'AUTO').toUpperCase() === 'MANUAL' &&
    game?.edge_state === 'ACTIVE'
  );
}

function displayedModelValue(game,market){
  if(manualDisplayActive(game)){
    const manual=game?.operator_model?.manual?.[market];
  ...
  const authority=game?.authority?.[market]?.value;
  const authorityNumber=Number(authority);

  if(Number.isFinite(authorityNumber)){
    return authorityNumber;
  }

  const fallback=market==='spread'
...
function displayedEdge(game,market){
  if(manualDisplayActive(game)){
    const manual=game?.operator_model?.manual?.[market];
    const value=Number(manual?.value);
    const edge=game?.operator_model?.manual_edges?.[market];

    if(Number.isFinite(value) && edge){
      return edge;
    }
  }

  return game?.edges?.[market] || null;
}

function manualModelLabel(game,market){
  if(!manualDisplayActive(game)) return '';

  const manual=game?.operator_model?.manual?.[market];

  if(!manual || !Number.isFinite(Number(manual.value))){
    return '';
  }

  const sources=manual.participating_sources || [];

  if(!sources.length) return '';

  return `MANUAL · ${sources.map(source=>{
    if(source==='TeamRankings') return 'TR';
    if(source==='DRatings Total') return 'DR';
    if(source==='Massey Dual') return 'MAS';
    return source;
  }).join(' + ')}`;
}
  ...
  const rows = components.map(name=>{
    ...
    const freshnessClass = missing
      ...
      : freshnessRow.participating !== true
        ...
        : ['CURRENT','UPDATED','PRE_GAME'].includes(freshnessState)
          ? 'updated'
          : ['STALE','CARRY_FORWARD','DEGRADED'].includes(freshnessState)
            ? 'stale'
            : 'available';
    ...
    if(manual){
      if(Number.isFinite(manualWeight)){
        weightText=`${(manualWeight*100).toFixed(1)}%`;
      }else if(requested.has(name)){
        weightText='SELECTED · MISSING';
      }else{
        weightText='OFF';
      }
...
function positionModelTooltip(trigger){
  ...
  panel.classList.add('viewport-tooltip-panel');

  document.body.appendChild(panel);
  trigger._viewportTooltip=panel;
  ...
  // Prefer below for top rows; otherwise use whichever side fully fits.
...
function closeModelTooltip(trigger){
  const panel=trigger?._viewportTooltip;

  if(panel){
    panel.remove();
    trigger._viewportTooltip=null;
  }
...
function recentBestMovePanel(game,market){
  ...
  const haveOldLine=Number.isFinite(oldLine);
  const haveNewLine=Number.isFinite(newLine);
  const haveOldPrice=Number.isFinite(oldPrice);
  const haveNewPrice=Number.isFinite(newPrice);

  if(!haveOldLine && !haveNewLine && !haveOldPrice && !haveNewPrice) return '';

  const lineText=(line)=>{
    if(!Number.isFinite(line)) return '—';
    return market==='spread' ? fmtLine(line) : Number(line).toFixed(1);
  };
  ...
  const oldQuote=`${lineText(oldLine)}${priceText(oldPrice)}`.trim();
  const newQuote=`${lineText(newLine)}${priceText(newPrice)}`.trim();

  const oldLabel=oldBook ? (BOOK_ABBR[oldBook] || oldBook) : '';
  const newLabel=newBook ? (BOOK_ABBR[newBook] || newBook) : '';
  ...
  return `<span class="best-latest-move">
    <span class="best-latest-move-label">LATEST MOVE</span>
    ${bookLine ? `<span class="best-latest-move-book">${esc(bookLine)}</span>` : ''}
    <span class="best-latest-move-value">${esc(oldQuote)} → ${esc(newQuote)}</span>
    ${timestamp ? `<span class="best-latest-move-time">${esc(fmtDateTimeET(timestamp))}</span>` : ''}
  </span>`;
}
...
function bestBookTooltip(game,market,side,bestQuote){
  ...
  const latestMove=recentBestMovePanel(game,market);
...
function mobileEdgeDisplay(game,market,side,edge){
  ...
  const team=side==='away'?game.away_team:game.home_team;
  let recommendation='';
  if(value!==null && value>0 && market==='spread' && side){
    const slug=teamLogoSlug(team);
    recommendation=`<span class="team-logo-holder" title="Bet ${esc(team)}"><img src="logos/${esc(slug)}.png" alt="${esc(team)}" onerror="this.parentElement.style.display='none'"></span>`;
  }else if(value!==null && value>0 && market==='total' && side){
    recommendation=`<span class="mobile-edge-direction">${side==='under'?'UNDER':'OVER'}</span>`;
  }
  const shown=`<span class="mobile-edge-display"><strong class="mobile-edge-value">${edgeDisplay(edge)}</strong>${recommendation}</span>`;
...
function renderMobileMatrix(rows){
  ...
  mobile.innerHTML=`<div class="mobile-command-table">
    ...
    const totSide=displayedTotalEdge?.best_side;
    const sprEdge=sprSide?displayedSpreadEdge?.best_edge:null;
    const totEdge=totSide?displayedTotalEdge?.best_edge:null;
...
function renderMatrix(){
  ...
  body.innerHTML = rows.map(game=>{
    ...
    const displayedSpreadEdge =
      displayedEdge(game,'spread');

    const displayedTotalEdge =
      displayedEdge(game,'total');

    const sprSide =
      displayedSpreadEdge?.best_side;

    const totSide =
      displayedTotalEdge?.best_side;

    const sprEdge =
      sprSide
        ? displayedSpreadEdge?.best_edge
        : null;

    const totEdge =
      totSide
        ? displayedTotalEdge?.best_edge
        : null;
...
function bestChangeTooltip(game,market){
  const event=recentBestEvent(game,market);
  if(!event) return '';
  ...
  const oldQuote=`${lineText(oldLine)}${priceText(oldPrice)}`;
  const newQuote=`${lineText(newLine)}${priceText(newPrice)}`;

  const bookText=
    oldBook && newBook && oldBook!==newBook
      ? `${oldBook} → ${newBook}`
      : (newBook || oldBook || 'Sportsbook unavailable');
  ...
  const timestamp=
    event.observed_at || event.detected_at || event.created_at;

  return [
    label,
    bookText,
    `${oldQuote} → ${newQuote}`,
    timestamp ? fmtDateTimeET(timestamp) : ''
  ].filter(Boolean).join(' · ');
}
...
function renderCoachCard(coach,game){
  ...
  let coachLogoSlug='';
  try{
    coachLogoSlug=teamLogoSlug(coach.team);
  }catch(err){
    console.error('Coach logo render failed',coach?.team,err);
  }

  // Some current coaches legitimately have no matched historical sample.
  // Preserve the standard FULL / 1H / 2H fields and render blanks rather
  // than throwing on null period rows or replacing the card entirely.
  const validPeriods=(coach.periods || []).filter(row=>row && typeof row==='object');
  const periodSource=validPeriods.length ? validPeriods : [
    {period:'full_game'},
    {period:'first_half'},
    {period:'second_half'}
  ];

  const periods=periodSource.map(row=>{
  ...
  const matchedRoleRows=(coach.role_splits || [])
    .filter(row=>row && String(row.role || '').toLowerCase()===String(role).toLowerCase())
    .sort((a,b)=>{
      const order={'Full Game':0,'1H':1,'2H':2};
      return (order[a.period] ?? 9)-(order[b.period] ?? 9);
    });

  const roleSource=matchedRoleRows.length ? matchedRoleRows : [
    {period:'Full Game',role},
    {period:'1H',role},
    {period:'2H',role}
  ];

  const roleRows=roleSource.map(row=>{
  ...
  return `<div class="coach-card">
    <div class="coach-head">
      ...
      <span class="coach-team-stack">
        <span class="coach-team">${esc(coach.team || '')}</span>
        <img
          class="coach-team-logo"
          src="logos/${esc(coachLogoSlug)}.png"
          alt="${esc(coach.team || '')}"
          title="${esc(coach.team || '')}"
...
function renderSignalsPanel(game){
  ...
  const coachByTeam=new Map(
    coaches
      ...
      .map(row=>[String(row.team),row])
  );

  const coachHtml=[game.away_team,game.home_team]
    .map(team=>renderCoachCard(
      coachByTeam.get(String(team)) || {
        team,
        coach:null,
        periods:[],
        role_splits:[]
...
function visibleActivity(){
  ...
  const source=SELECTED_GAME_ID
    ? (ACTIVITY_FILTER==='POSTGAME'
      ? [...selectedEvents.filter(event=>activityCategory(event)==='POSTGAME'),...priorEvents]
      : selectedEvents)
    : (ACTIVITY?.events || []);
...
function renderMarketSnapshot(game,gameData){
  ...
  const openerLine=(label,opener,market)=>{
    if(!opener){
      return `<div class="snapshot-open-row">
        <div class="snapshot-open-kind">${label}</div>
        <div class="snapshot-open-value">OPENING LINE NOT CAPTURED</div>
      </div>`;
    }

    const line=market==='spread'
      ? `${esc(game.home_team)} ${fmtLine(opener.line)}${priceText(opener.price)}`
      : `${Number(opener.line).toFixed(1)}${priceText(opener.price)}`;

    return `<div class="snapshot-open-row">
      <div class="snapshot-open-kind">${label}</div>
      <div class="snapshot-open-value">${line}</div>
      <div class="snapshot-open-meta">${bookText(opener.book)} · ${fmtDateTimeET(opener.observed_at)}</div>
    </div>`;
  };
  ...
  return `<div class="snapshot-title">MARKET HISTORY</div>
    ...
    <div class="snapshot-opener-box">
      <div class="snapshot-market-section-title">OPENERS</div>
      ${openerLine('SPREAD',spreadOpen,'spread')}
...
function priorStatusLine(prior){
  ...
  const opponent=prior.selected_team===prior.home_team
    ? prior.away_team
    : prior.home_team;
  ...
  if(
    prior.status!=='FINAL_POSTED' ||
    awayScore===null || awayScore===undefined ||
    homeScore===null || homeScore===undefined
  ){
    return `${opponent?`vs ${opponent}`:'OPPONENT UNAVAILABLE'} · RESULT UNAVAILABLE`;
  }

  const selectedScore=prior.selected_team===prior.home_team ? homeScore : awayScore;
  const opponentScore=prior.selected_team===prior.home_team ? awayScore : homeScore;

  const outcome=Number(selectedScore)>Number(opponentScore)
    ? 'W'
    : Number(selectedScore)<Number(opponentScore)
      ? 'L'
      : 'T';

  const date=prior.date || prior.kickoff_time;
  const dateText=date ? ` · ${fmtKickoffDateET(date)}` : '';

  return `${opponent?`vs ${opponent}`:'OPPONENT UNAVAILABLE'} · ${outcome} ${selectedScore}-${opponentScore} · FINAL${dateText}`;
}
...
function renderSelectedSnapshot(){
  ...
  if(
    !game ||
    !SELECTED_GAME_ID ||
    !['MARKET','POSTGAME'].includes(ACTIVITY_FILTER)
  ){
...
document.getElementById('teamSearch').addEventListener('input',e=>{
  ...
  const mobileSearch=document.getElementById('mobileTeamSearch');
  if(mobileSearch && mobileSearch.value!==SEARCH_QUERY){
    mobileSearch.value=SEARCH_QUERY;
  }
  reconcileSelectedGame();
  renderHealth();
  renderMatrix();
...
document.getElementById('mobileTeamSearch').addEventListener('input',e=>{
  SEARCH_QUERY=String(e.target.value || '');
  const desktopSearch=document.getElementById('teamSearch');
  if(desktopSearch && desktopSearch.value!==SEARCH_QUERY){
    desktopSearch.value=SEARCH_QUERY;
  }
  reconcileSelectedGame();
  renderHealth();
  renderMatrix();
...
function requestViaRelay(action, button, old, payload=null, lifecycle={}){
  ...
  RELAY_REQUESTS.set(requestId,{button,old,lifecycle});
  CONTROL_WINDOW.postMessage(
    {
      channel:CONTROL_CHANNEL,
      channelNonce:ensureControlNonce(),
      type:'REQUEST',
      requestId,
      action,
      payload
...
addEventListener('message',event=>{
  ...
  if(message.type==='ACK'){
    ...
    if(terminal.has(message.task?.status)){
      const succeeded=message.task.status==='COMPLETED' || message.task.status==='COMPLETED_WITH_WARNINGS';
      if(succeeded){
        Promise.resolve(loadData()).then(()=>pending.lifecycle.onSuccess?.(message.task));
      }else{
        pending.lifecycle.onError?.(message.task?.error || message.task?.status || 'Task failed');
      }
    ...
    pending.lifecycle.onError?.(message.message || 'Operator session expired');
    CONTROL_WINDOW=null;
  }else if(message.type==='ERROR'){
    pending.button.textContent='⚠ REQUEST FAILED';
    status.textContent=`Request failed · ${message.message}`;
    RELAY_REQUESTS.delete(message.requestId);
    pending.lifecycle.onError?.(message.message || 'Request failed');
    setTimeout(()=>{pending.button.textContent=pending.old;pending.button.disabled=false},2500);
  }
});

function requestOperation(action, button, runningLabel, payload=null, lifecycle={}){
  const old = button.textContent;
  button.disabled = true;
  button.textContent = runningLabel;
  const status = document.getElementById('operatorStatus');
  status.textContent = 'Submitting authenticated request…';
  try{requestViaRelay(action,button,old,payload,lifecycle)}catch(err){
    button.textContent='⚠ REQUEST FAILED';
    status.textContent = `Request failed · ${err.message}`;
    lifecycle.onError?.(err.message);
...
function currentOperatorModel(){
  const game=(MATRIX?.games || []).find(g=>g?.operator_model);
  return game?.operator_model || {
    mode:'AUTO',
    spread_sources:['SP+','FPI','TeamRankings','DRatings'],
    total_sources:['SP+','Massey Dual','DRatings Total']
  };
}
...
function setManualCheckboxes(selector,selected){
...
function syncModelModeControls(){
  const config=currentOperatorModel();
  ...
  if(config.spread_sources){
    setManualCheckboxes('.manual-spread-source',config.spread_sources);
  }

  if(config.total_sources){
...
function submitModelOverride(mode,button){
  ...
  const panel=document.getElementById('manualSourcePanel');
  const persistedMode=String(currentOperatorModel().mode || 'AUTO').toUpperCase();
  const auto=document.getElementById('modelAutoBtn');
  const manual=document.getElementById('modelManualBtn');
  const status=document.getElementById('manualSourceStatus');

  if(auto) auto.classList.toggle('active',mode==='AUTO');
  if(manual) manual.classList.toggle('active',mode==='MANUAL');
  if(panel) panel.hidden=mode!=='MANUAL';
  if(status) status.textContent=mode==='MANUAL' ? 'Applying selected sources…' : 'Switching to automatic model…';

  requestOperation(
    'model-override',
    button,
    mode==='AUTO' ? 'AUTO…' : 'APPLYING…',
    payload,
    {
      onSuccess:()=>{
...
document.getElementById('modelAutoBtn').addEventListener('click',e=>{
  document.getElementById('manualSourcePanel').hidden=true;
  submitModelOverride('AUTO',e.currentTarget);
});
...
document.getElementById('manualCancelBtn').addEventListener('click',()=>{
  ...
  setManualCheckboxes(
    '.manual-spread-source',
    config.spread_sources || []
  );
...
document.getElementById('manualApplyBtn').addEventListener('click',e=>{
  submitModelOverride('MANUAL',e.currentTarget);
});
