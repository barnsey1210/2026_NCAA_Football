...
# Permit only known generated runtime artifacts to be dirty. These are
# regenerated by the daily pipeline and/or synchronized from validated AUTO
# public artifacts. Any dirty source/config/deploy file still blocks publish.
GENERATED_DIRTY=""
DOC_DIRTY=""
UNSAFE_DIRTY=""
...
if [[ -n "$UNSAFE_DIRTY" ]]; then
  printf '%s' "$UNSAFE_DIRTY" >&2
  die "canonical repository has unsafe tracked local changes; review them before the daily publish"
fi
...
cleanup(){
  ...
  if [[ "$DOC_STASHED" = "1" ]]; then
    ...
    if ! git -C "$MAIN_REPO" stash pop --quiet; then
      log "ERROR: documentation stash could not be restored cleanly"
      [[ $rc -ne 0 ]] || rc=1
    fi
  fi
...
if [[ -n "$DOC_DIRTY" ]]; then
  log "temporarily stashing tracked documentation edits"
  git -C "$MAIN_REPO" stash push \
    -m "canonical-publish-docs-$(date -u +%Y%m%dT%H%M%SZ)" \
    -- docs >/dev/null
  DOC_STASHED=1
fi

if [[ -n "$GENERATED_DIRTY" ]]; then
  log "discarding tracked generated artifacts in canonical repository before publish"
  printf '%s' "$GENERATED_DIRTY"

  while IFS= read -r line; do
    [[ -n "$line" ]] || continue
    path="${line:3}"
    git -C "$MAIN_REPO" restore -- "$path"
  done <<< "$GENERATED_DIRTY"
fi
