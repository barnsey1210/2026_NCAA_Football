...
def raw_team_key(value):
 text=unicodedata.normalize('NFKD',str(value or ''));text=''.join(ch for ch in text if not unicodedata.combining(ch));return re.sub(r'[^a-z0-9]+',' ',text.lower()).strip()
def alias_keys():
 path=ROOT/'config/team_aliases.json';rows=json.loads(path.read_text()).get('aliases',{}) if path.exists() else {};return {raw_team_key(alias):raw_team_key(target) for alias,target in rows.items()}
TEAM_ALIASES=alias_keys()
def team_key(value):
 key=raw_team_key(value);return TEAM_ALIASES.get(key,key)
def kickoff_index():
 path=ROOT/'data/canonical/cfbd_schedule_2026.json'
 if not path.exists():return {}
 rows=json.loads(path.read_text()).get('games',[])
 return {(r.get('week'),team_key(r.get('away_team')),team_key(r.get('home_team'))):r.get('start_date') for r in rows if r.get('start_date')}
def with_exact_kickoff(g,index):
 kickoff=index.get((g.get('week'),team_key(g.get('away_team')),team_key(g.get('home_team'))))
 return {**g,'kickoff_at':kickoff} if kickoff else g
def future_trackable(g,now):
 if str(g.get('availability_status') or '').upper() in {'CLOSING','CLOSED','FINAL'}:return False
 kickoff=g.get('kickoff_at') or g.get('date')
 if not kickoff:return False
 try:
  parsed=datetime.fromisoformat(str(kickoff).replace('Z','+00:00'));parsed=parsed.replace(tzinfo=timezone.utc) if parsed.tzinfo is None else parsed;return parsed>now
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--accept',action='store_true');args=ap.parse_args();pc=ROOT/'data/site/current_game_projection_contract.json';mc=ROOT/'data/site/current_market_contract.json';p=json.loads(pc.read_text());m=json.loads(mc.read_text());pred=[];markets=[];decisions=[];market_states=[];market_confirmations=[];decision_states=[];decision_confirmations=[];market_index={};now=datetime.now(timezone.utc);kickoffs=kickoff_index();projection_games=[with_exact_kickoff(g,kickoffs) for g in p.get('games',[])];prediction_eligible={str(g['game_id']) for g in projection_games if future_trackable(g,now)};market_eligible={str(g['game_id']) for g in m.get('games',[]) if future_trackable(g,now)}
 for g in projection_games:
  if str(g['game_id']) not in prediction_eligible:continue
 ...
 for g in m.get('games',[]):
  if str(g['game_id']) not in market_eligible:continue
 ...
 for g in m.get('games',[]):
  ...
  for market in ('spread','total'):
   ...
   for pr in projection_by_game.get((g['game_id'],market),[]):
    ...
    edge=abs(float(pr['projection'])+float(mo['line'])) if market=='spread' else abs(float(pr['projection'])-float(mo['line']));did=stable_id('decision',pr['observation_id'],mo['observation_id'],side,edge);decision={'decision_id':did,'prediction_observation_id':pr['observation_id'],'market_observation_id':mo['observation_id'],'canonical_game_id':g['game_id'],'market_type':market,'checkpoint':mo.get('lifecycle_state') or 'CURRENT_ACCEPTED','bet_side':side,'edge':edge,'market_provenance':{'sportsbook':book,'source':mo.get('source'),'freshness_status':mo.get('freshness_status')},'created_at':datetime.now(timezone.utc).isoformat()};decisions.append(decision);decision_state=decision_state_row(decision,mo);decision_states.append(decision_state);decision_confirmations.append(decision_confirmation_row(decision,mo,decision_state['decision_state_id']))
 report={'schema_version':'tracking-capture-preview-v3','generated_at':datetime.now(timezone.utc).isoformat(),'accept_requested':args.accept,'eligible_future_games':len(prediction_eligible),'prediction_eligible_future_games':len(prediction_eligible),'market_eligible_future_games':len(market_eligible),'eligibility_policy':'predictions use projection-contract games with future exact kickoff; market and decisions independently require accepted market games','source_contracts':[str(pc.relative_to(ROOT)),str(mc.relative_to(ROOT))],'predictions':append_unique(DEST/'prediction_observations.jsonl',pred,'observation_id',args.accept),'markets':{'path':str(DEST/'market_observations.jsonl'),'candidates':len(markets),'already_present':None,'would_append':0,'accepted':0,'write_status':'DISABLED_NORMALIZED_AUTHORITY'},'decisions':{'path':str(DEST/'decision_observations.jsonl'),'candidates':len(decisions),'already_present':None,'would_append':0,'accepted':0,'write_status':'DISABLED_NORMALIZED_AUTHORITY'},'market_states':append_unique(DEST/'market_states.jsonl',market_states,'market_state_id',args.accept),'market_confirmations':append_unique(DEST/'market_confirmations.jsonl',market_confirmations,'confirmation_id',args.accept),'decision_states':append_unique(DEST/'decision_states.jsonl',decision_states,'decision_state_id',args.accept),'decision_confirmations':append_unique(DEST/'decision_confirmations.jsonl',decision_confirmations,'confirmation_id',args.accept)};print(json.dumps(report,indent=2))
