...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--accept',action='store_true');args=ap.parse_args();pc=ROOT/'data/site/current_game_projection_contract.json';mc=ROOT/'data/site/current_market_contract.json';p=json.loads(pc.read_text());m=json.loads(mc.read_text());pred=[];markets=[];decisions=[];market_states=[];market_confirmations=[];decision_states=[];market_index={};now=datetime.now(timezone.utc);eligible={str(g['game_id']) for g in m.get('games',[]) if future_trackable(g,now)}
