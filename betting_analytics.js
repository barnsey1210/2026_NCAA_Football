(()=>{
'use strict';
const CONTRACT='data/site/historical_betting_analytics_v2.json?v=20260923T024320Z',ORDER_FALLBACK=[],$=id=>document.getElementById(id),pct=v=>v==null?'N/A':(Number(v)*100).toFixed(1)+'%',signed=(v,d=2)=>v==null?'N/A':(Number(v)>0?'+':'')+Number(v).toFixed(d),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let C;
...
(()=>{'use strict';const P='data/site/historical_betting_explorer_v1.json?v=20260923T024320Z',A='data/site/historical_betting_analytics_v2.json?v=20260923T024320Z',q=id=>document.getElementById(id);if(!q('historicalExplorer'))return;let X,H,R=[];const dims={week:r=>'W'+r.week,checkpoint:r=>r.checkpoint.replaceAll('_ET','').replaceAll('_',' '),season:r=>String(r.season),edge:r=>{const e=Math.abs(r.edge);return e>=10?'10+':e>=8?'8–9.99':e>=6?'6–7.99':e>=5?'5–5.99':e>=4?'4–4.99':e>=3?'3–3.99':e>=2?'2–2.99':e>=1?'1–1.99':'0.5–0.99'}};const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
