...
def load_site_teams():
    html = SITE_INDEX.read_text(errors="ignore")
    m = re.search(r'<script id="db" type="application/json">(.*?)</script>', html, re.S)
    if not m:
        raise SystemExit("Could not find DB in index.html")
    db = json.loads(m.group(1))
    teams = pd.DataFrame(db["teams"])
    return teams[["team", "conference"]].copy()
...
TEAM_ALIASES = {
    ...
    # 2026 new FBS teams / ESPN FPI variants
...
def load_spplus():
    path = Path("data/ratings/raw/spplus/spplus_table_0.csv")
    source = "SP+"
    if not path.exists():
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            ...
            "pulled_at": now_utc(),
            "source_updated_at": "",
            "notes": "SP+ table_0; rating=sp.",
        })
    return rows, {"source": source, "status": "ok", "path": str(path), "rows": len(rows)}

def load_fpi():
    path_team = Path("data/ratings/raw/fpi/fpi_table_0.csv")
    path_vals = Path("data/ratings/raw/fpi/fpi_table_1.csv")
    source = "FPI"
    if not path_team.exists() or not path_vals.exists():
        return [], {"source": source, "status": "missing", "path": f"{path_team}|{path_vals}", "rows": 0}
    teams = pd.read_csv(path_team)
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "pulled_at": now_utc(),
            "source_updated_at": "",
            "notes": "FPI table_0 + table_1 merged by row order; rating=power_index_fpi.",
        })
    return rows, {"source": source, "status": "ok", "path": f"{path_team}|{path_vals}", "rows": len(rows)}

def load_teamrankings():
    path = Path("data/ratings/raw/teamrankings/teamrankings_table_0.csv")
    source = "TeamRankings"
    if not path.exists():
        return [], {"source": source, "status": "missing", "path": str(path), "rows": 0}
    df = pd.read_csv(path)
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            ...
            "pulled_at": now_utc(),
            "source_updated_at": "",
            "notes": "TeamRankings table_0; rating=rating.",
        })
    return rows, {"source": source, "status": "ok", "path": str(path), "rows": len(rows)}
...
def load_brad_powers():
    path = Path("data/ratings/bradpowers_2025_test_latest.csv")
    source = "Brad Powers"
    if not path.exists():
        return [], {"source": source, "status": "missing", "path": str(path), "rows": 0}
