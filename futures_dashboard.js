(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.FuturesDashboard=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
'use strict';
const MODES={wins:{domain:'win',edge:'win_edge'},title:{domain:'conference_title',edge:'title_edge'},cfp:{domain:'make_cfp',edge:'playoff_edge'},national:{domain:'national_title',edge:'national_title_edge'}},BOOKS=['DraftKings','FanDuel','BetMGM','Caesars'];
const hasNumber=v=>v!==null&&v!==undefined&&v!==''&&Number.isFinite(Number(v));
const focusMode=s=>s.mode==='playoff'?(s.playoffFocus==='title'?'national':'cfp'):s.mode;
function spec(mode){return MODES[mode]||MODES.wins}function currentEdge(row,mode){const v=row?.[spec(mode).edge];return hasNumber(v)?Number(v):null}
function delta(row,mode,period,key='edge'){const v=row?.[period==='day'?'delta_day':'delta_week']?.[spec(mode).domain]?.[key];return hasNumber(v)?Number(v):null}
function priorEdge(row,mode,period){const now=currentEdge(row,mode),change=delta(row,mode,period);return now===null||change===null?null:now-change}
function deltas(row,mode,period){return row?.[period==='day'?'delta_day':'delta_week']?.[spec(mode).domain]||{}}
function metric(row,mode,period,key){return key==='edge'?currentEdge(row,mode):key==='edge_change'?delta(row,mode,period):delta(row,mode,period,key==='model_change'?'model':'market')}
function quoteFor(quotes,book){return quotes?.[book]||null}function isBestBook(row,mode,book){return row?.[{wins:'win_book',title:'title_book',cfp:'playoff_book',national:'national_title_book'}[mode]]===book}
...
function rating(x){if(!F.hasNumber(x.team_rating))return'—';const d=x.team_rating_delta_week;return`<b>${num(x.team_rating)}</b>${F.hasNumber(d)?` <small class="${d>0?'moveUp':d<0?'moveDown':'neutral'}">${d>0?'↑':d<0?'↓':'→'} ${d>0?'+':''}${num(d)}</small>`:''}`}
function teamCell(x){return`<td class="teamColumn"><button class="teamOpen" data-team="${esc(x.team)}">${logo(x)}<span><b>${esc(x.team)}</b><small>${esc(x.conference||'IND')} ${x.record?.wins??'—'}-${x.record?.losses??'—'}</small></span></button></td>`}
function quoteCell(x,book,kind){const q=F.quoteFor(x[`${kindPrefix(kind)}_quotes`],book),best=F.isBestBook(x,kind,book);if(!q)return`<td class="bookQuote missing" data-label="${book}">—</td>`;const side=kind==='wins'?x.win_direction:'Win',price=kind==='wins'?(side==='Under'?q.under_price:q.over_price):q.price,primary=kind==='wins'?num(q.number):percent(q.implied_probability);return`<td class="bookQuote${best?' bestBook':''}" data-label="${book}" title="${esc(book)} · ${esc(side)}${best?' · selected best executable quote':''}"><span>${primary}</span><b>${odds(price)}</b>${best?'<i aria-label="Best executable">●</i>':''}</td>`}
function modelValue(x,k){return k==='wins'?num(x.projected_wins):k==='title'?percent(x.title_model_prob):k==='cfp'?percent(x.playoff_model_prob):percent(x.national_title_model_prob)}
...
function chart(x){if(state.historyMetric==='rating')return ratingHistory(x);const k=modeKey(),prob=k!=='wins',slots=F.historySlots(x,F.spec(k).domain,D.history_axis?.labels,D.weekly_baseline?.checkpoint_date,D.history_axis?.current_week),values=slots.flatMap(p=>[p.model,p.market]).filter(F.hasNumber).map(v=>Number(v)*(prob?100:1));if(!values.length)return'<p class="railEmpty">History unavailable; missing values remain gaps.</p>';let min=Math.min(...values),max=Math.max(...values);if(min===max){min-=1;max+=1}const w=560,h=190,l=44,r=10,t=12,b=36,px=i=>l+i*(w-l-r)/Math.max(1,slots.length-1),py=v=>t+(max-v)*(h-t-b)/(max-min),segments=key=>{let paths=[],part=[];slots.forEach((p,i)=>{if(F.hasNumber(p[key]))part.push(`${px(i)},${py(Number(p[key])*(prob?100:1))}`);else if(part.length){paths.push(part);part=[]}});if(part.length)paths.push(part);return paths.map(a=>`<polyline points="${a.join(' ')}"></polyline>`).join(' ')},points=key=>slots.map((p,i)=>F.hasNumber(p[key])?`<circle cx="${px(i)}" cy="${py(Number(p[key])*(prob?100:1))}" r="4"><title>${p.label} · ${key} ${prob?percent(p[key]):num(p[key])} · edge ${F.hasNumber(p.model)&&F.hasNumber(p.market)?(prob?((p.model-p.market)*100).toFixed(1)+' pts':num(p.model-p.market)):'—'}</title></circle>`:'').join('');return`<svg class="historyChart" viewBox="0 0 ${w} ${h}" role="img"><g class="grid"><line x1="${l}" y1="${t}" x2="${l}" y2="${h-b}"></line><line x1="${l}" y1="${h-b}" x2="${w-r}" y2="${h-b}"></line></g><text x="2" y="${t+4}">${max.toFixed(1)}${prob?'%':''}</text><text x="2" y="${h-b}">${min.toFixed(1)}${prob?'%':''}</text><g class="modelSeries">${segments('model')}${points('model')}</g><g class="marketSeries">${segments('market')}${points('market')}</g>${slots.map((p,i)=>`<text class="axisLabel" x="${px(i)}" y="${h-13}" text-anchor="middle">${p.label}</text>`).join('')}</svg><div class="chartLegend"><span>● Model</span><span>● Market</span><small>Blank weeks are intentional gaps.</small></div>`}
function ratingHistory(x){const p=x.team_rating_history||[];return`<div class="ratingHistory">${p.map((v,i)=>{const d=i?Number(v.rating)-Number(p[i-1].rating):null;return`<div><span>${esc(v.date)}</span><b>${num(v.rating)}</b><b class="${d>0?'moveUp':d<0?'moveDown':'neutral'}">${d===null?'—':`${d>0?'↑':d<0?'↓':'→'} ${d>0?'+':''}${num(d)}`}</b></div>`}).join('')||'<p class="railEmpty">Authentic rating history unavailable.</p>'}</div>`}
function renderRail(){const x=activeTeam;if(!x)return;rail.innerHTML=`<div class="railHead"><div>${logo(x)}<span><b>${esc(x.team)}</b><small>${x.record?.wins??'—'}-${x.record?.losses??'—'} · ${esc(x.conference||'IND')} · Rating ${num(x.team_rating)} ${F.hasNumber(x.team_rating_delta_week)?`${x.team_rating_delta_week>0?'↑':x.team_rating_delta_week<0?'↓':'→'} ${x.team_rating_delta_week>0?'+':''}${num(x.team_rating_delta_week)}`:''}</small></span></div><button id="railClose">×</button></div><nav class="railTabs">${['overview','schedule','history'].map(t=>`<button data-rail="${t}" class="${state.railTab===t?'active':''}">${t.toUpperCase()}</button>`).join('')}</nav>${state.railTab==='history'?`<div class="historyToggle"><button data-history-metric="futures" class="${state.historyMetric==='futures'?'active':''}">FUTURES</button><button data-history-metric="rating" class="${state.historyMetric==='rating'?'active':''}">RATING</button></div>`:''}<section class="railBody ${state.railTab}">${state.railTab==='schedule'?schedule(x):state.railTab==='history'?chart(x):overview(x)}</section>`;rail.hidden=false;document.body.classList.add('railOpen');railClose.onclick=closeRail;rail.querySelectorAll('[data-rail]').forEach(b=>b.onclick=()=>{state.railTab=b.dataset.rail;renderRail()});rail.querySelectorAll('[data-history-metric]').forEach(b=>b.onclick=()=>{state.historyMetric=b.dataset.historyMetric;renderRail()});if(innerWidth<=900)requestAnimationFrame(()=>rail.scrollIntoView({block:'start'}))}
