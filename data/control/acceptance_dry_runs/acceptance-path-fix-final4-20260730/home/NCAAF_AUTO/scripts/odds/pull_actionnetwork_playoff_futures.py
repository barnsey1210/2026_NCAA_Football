...
def brand(book):
    ...
    for needle, label in (("draftkings","DraftKings"),("betmgm","BetMGM"),("playmgm","BetMGM"),("fanduel","FanDuel"),("caesars","Caesars"),("bet365","bet365"),("betrivers","BetRivers"),("hard rock","Hard Rock")):
        if needle in text: return label
    if re.search(r"\bdk\b", text): return "DraftKings"
...
def main():
    ...
    if missing: raise SystemExit("Action futures markets missing: " + ", ".join(missing))
    books_payload = fetch(BOOKS)
    books = {str(x.get("id")): brand(x) for x in books_payload.get("books", []) if x.get("id") is not None}
    markets = {}
    for key, market_type in WANTED.items():
        markets[key] = fetch(f"https://api.actionnetwork.com/web/v1/leagues/2/futures/{market_type}")
    payload = {"schema_version":"action-playoff-futures-v1","pull_succeeded":True,"pulled_at":pulled_at,"books":books,"markets":markets}
    OUT.parent.mkdir(parents=True, exist_ok=True)
    temp = OUT.with_suffix(".tmp")
    temp.write_text(json.dumps(payload, separators=(",",":")) + "\n")
    temp.replace(OUT)
    print(OUT)
    print({key: len(value.get("books", [])) for key, value in markets.items()})
