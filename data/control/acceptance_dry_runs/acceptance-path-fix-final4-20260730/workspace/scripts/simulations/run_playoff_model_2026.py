#!/usr/bin/env python3
"""Run the 2026-27 12-team CFP model from the canonical embedded site DB.
...
def load_db(path: Path):
    ...
    match = DB_RE.search(html)
    if not match:
        raise SystemExit(f"Embedded DB not found in {path}")
    return html, match, json.loads(match.group(2))


def write_db(path: Path, html: str, match: re.Match, db: dict):
    payload = json.dumps(db, separators=(",", ":"))
    path.write_text(html[:match.start(2)] + payload + html[match.end(2):], encoding="utf-8")
...
def game_winner(a: str, b: str, teams: Dict[str, dict], rng: random.Random, sigma: float, home: str | None = None) -> str:
    ...
    if home == b:
        margin_b += fnum(tb.get("hfa"), 2.5)
    elif home == a:
        margin_b -= fnum(ta.get("hfa"), 2.5)
    p_b = CONF.implied_home_prob_from_margin(margin_b, sigma)
...
def run_model(db: dict, sims: int, seed: int, sigma: float) -> dict:
    ...
    for _ in range(sims):
        ...
        for g, p_home in game_probs:
            ...
            if g.get("cfbd_completed") and g.get("away_score") is not None and g.get("home_score") is not None:
                ...
                if expected is None:
                    expected = CONF.estimate_margin_home(teams[home], teams[away], bool(g.get("neutral_site")))
                margin_home = rng.gauss(fnum(expected), sigma)
                if abs(margin_home) < 0.01:
                    margin_home = 0.01
        ...
        for conf, names in conf_teams.items():
            ...
            margin_b = rng.gauss(CONF.team_rating(teams[b]) - CONF.team_rating(teams[a]), sigma)
    ...
    db["playoff_model"] = {
        ...
        "metadata": {
            ...
            "game_model": f"Normal margin model, sigma={sigma}; first-round higher seed receives team HFA; later rounds neutral",
        },
        "projected_field": [
...
def main():
    ...
    html, match, db = load_db(path)
    db = run_model(db, args.sims, args.seed, args.sigma)
    out = ROOT / args.output
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(db["playoff_model"], indent=2), encoding="utf-8")
    if not args.dry_run:
        write_db(path, html, match, db)
    print(f"Playoff model: {args.sims} trials")
    print(f"Wrote {out}")
    print("Embedded DB updated" if not args.dry_run else "Dry run: embedded DB unchanged")
    for row in db["playoff_model"]["teams"][:15]:
        print(f"{row['team']:<22} CFP {row['playoff_pct']:.1%}  Bye {row['playoff_bye_pct']:.1%}  Title {row['national_title_pct']:.1%}")
