...
(function(){
  ...
  function installLineHistoryRoute(){
    ...
    if (!window.__lh2DataReadyHooked) {
      window.__lh2DataReadyHooked = true;
      window.addEventListener('matchup-line-history-ready', () => {
        if (location.hash !== '#line-history') return;
        const app = document.getElementById('app');
        if (app) app.innerHTML = renderLineHistoryStandalone();
        setTimeout(bindLH2, 0);
      });
    }
  }
...
def inject(path):
    ...
    # Add or normalize the route before Results Center without duplicating it.
    line_history_route = "else if (hash==='#line-history') html = typeof window.renderLineHistoryStandalone === 'function' ? window.renderLineHistoryStandalone() : '<div class=\"card\">Loading line history&hellip;</div>';"
    existing_route = re.compile(r"\s*else if \(hash==='#line-history'\) html = [^\n]*;")
    html = existing_route.sub("", html)
    html = html.replace(
    ...
    bind_after_render = "  if (location.hash==='#line-history' && typeof bindLH2 === 'function') setTimeout(bindLH2, 50);"
    html = re.sub(r"\n(?:  if \(location\.hash==='#line-history' && typeof bindLH2 === 'function'\) setTimeout\(bindLH2, 50\);)+", "", html)
    html = html.replace(
