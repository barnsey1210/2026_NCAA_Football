...
def main():
    ...
    if not moves.empty:
        ...
        for key, g in win_moves.groupby(group_cols, dropna=False):
            ...
            if len(over):
                o = over.iloc[0]
                pieces.append(f"Over {total}: {o.get('previous')} → {o.get('latest')}")
                scores.append(float(o.get("abs_imp") or 0))

            if len(under):
                u = under.iloc[0]
                pieces.append(f"Under {total}: {u.get('previous')} → {u.get('latest')}")
                scores.append(float(u.get("abs_imp") or 0))
