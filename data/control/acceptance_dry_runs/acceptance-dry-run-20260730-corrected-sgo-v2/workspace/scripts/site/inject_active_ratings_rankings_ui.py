...
DEFAULT_WEIGHTS = {key: 0.0 for key in LABELS}
DEFAULT_WEIGHTS.update({s["key"]: s["weight"] for s in MODEL["sources"]})
ACTIVE_KEYS = {s["key"] for s in MODEL["sources"]}
...
def build_payloads():
    ...
    for _, r in master.iterrows():
        ...
        master_payload[team] = {
            ...
            "bradpowers": None if pd.isna(r.get("bradpowers")) else round(float(r.get("bradpowers")), 4),
            "composite_status": clean_str(r.get("composite_status")),
        }
...
def update_db_teams(s, master_payload):
    ...
    for t in teams:
        ...
        t["rating_bradpowers"] = r["bradpowers"]
        t["rating_composite_status"] = r["composite_status"]
        t["rating_model_note"] = f"Active 2026 blend: {model_summary()}"
...
(function(){
  ...
  // Reset stale browser display-lab weights when the production source set changes.
  try {
    const versionKey = 'ratingLabModelVersion';
    const currentVersion = 'active-2026-brad-sp-fpi-teamrankings-v1';
  ...
  window.ratingSourceStatusGrid = function(){
    ...
    return `<div class="production-model-note">
      Production model: <b>Active 2026 ratings only</b> — __MODEL_SUMMARY__.
  ...
  window.rankingsPowerRatingCell = function(t) {
    ...
    if (!useLab) {
      return `<div class="rankings-power-cell">
        ...
        <div class="small muted">active 2026 blend · ${t.rating_source_count || 4}/4</div>
      </div>`;
    }
...
def inject_css_js(s):
    ...
    insert = "\n" + CSS + "\n" + JS.replace("__MODEL_SUMMARY__", model_summary()) + "\n"
    if "</body>" not in s:
        return s + insert
