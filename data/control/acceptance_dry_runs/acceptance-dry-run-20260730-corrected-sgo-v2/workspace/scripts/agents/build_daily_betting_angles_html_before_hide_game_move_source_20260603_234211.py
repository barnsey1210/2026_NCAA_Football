...
def build_game_line_move_cards(moves: pd.DataFrame, limit: int = 20) -> str:
    ...
    for _, r in df.iterrows():
        ...
        source_prev = esc(r.get("source_prev", ""))
        source_latest = esc(r.get("source_latest", ""))

        source_html = ""
        if source_prev or source_latest:
            if source_prev and source_latest and source_prev != source_latest:
                source_html = f'<span class="meta-pill">Source: {source_prev} → {source_latest}</span>'
            else:
                source_html = f'<span class="meta-pill">Source: {source_latest or source_prev}</span>'
        ...
        # Keep the source transition as metadata, not as the main reason.
        if game:
            clean_summary = f"{game} {market.lower()} moved {previous} → {latest}."
        else:
...
"""
    ...
    print(f"Wrote {OUT}: {move_count} market moves, {game_count} game line edges, {arb_count} arbitrage opportunities")
