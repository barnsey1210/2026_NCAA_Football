...
def main():
    ...
    for path in FILES:
        ...
        for g in games:
            ...
            # A schedule/season-lines source may already have a valid opener.
            # Retain it when SGO has not supplied an opening number.
            prior_spread_open = clean(g.get("market_spread_open_home"))
            prior_total_open = clean(g.get("market_total_open"))

            for f in CLEAR_FIELDS:
                g.pop(f, None)

            if not r:
                if prior_spread_open is not None:
                    g["market_spread_open_home"] = prior_spread_open
                if prior_total_open is not None:
                    g["market_total_open"] = prior_total_open
            ...
            vals = {
                ...
                "market_spread_open_home": (
                    clean(r.get("market_spread_open_home"))
                    if clean(r.get("market_spread_open_home")) is not None
                    else prior_spread_open
                ),
                ...
                "market_total_open": (
                    clean(r.get("market_total_open"))
                    if clean(r.get("market_total_open")) is not None
                    else prior_total_open
                ),
