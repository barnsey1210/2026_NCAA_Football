...
def parse_rp_text(path, season):
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Create it by copying the {season} table and running pbpaste > {path}"
        )
    ...
    if parsed.empty:
        raise ValueError(
            f"No rows parsed from {path}. "
            "Confirm the pasted lines still contain percentages and ranks."
        )
