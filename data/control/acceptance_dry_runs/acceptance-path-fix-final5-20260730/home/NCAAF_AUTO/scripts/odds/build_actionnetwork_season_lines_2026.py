...
def standardized_spread_hold_pct(home_line, home_price, away_line, away_price):
    """
    Calculate synthetic sportsbook hold from the two best available spread sides.

    This is a market-shopping indicator:
      lower hold = less sportsbook cushion
      negative hold = bettor edge / arbitrage condition

    Raw hold:
      implied(home best price) + implied(away best price) - 100

    Then adjust for spread gap/overlap:
      home_line + away_line > 0 lowers hold because bettor has a spread gap cushion
      home_line + away_line < 0 raises hold because bettor faces an overlap/dead-zone cost

    Display remains "Hold +/-x.x%" with no middle wording.
    """
    try:
    ...
    raw_hold = (h_prob + a_prob - 1.0) * 100.0

    spread_gap = h_line + a_line
    half_points = abs(spread_gap) / 0.5

    # Conservative CFB spread half-point value for a broad market-board indicator.
    # Lower this if the hold still looks too punitive; raise if it overstates arbs.
    HALF_POINT_VALUE_PCT = 1.0

    if spread_gap > 0:
        adjusted_hold = raw_hold - (half_points * HALF_POINT_VALUE_PCT)
    elif spread_gap < 0:
        adjusted_hold = raw_hold + (half_points * HALF_POINT_VALUE_PCT)
    else:
        adjusted_hold = raw_hold

    # For reference only: standardized midpoint lines.
    target_home_line = round((h_line + (-a_line)) / 2.0, 2)
    target_away_line = round(-target_home_line, 2)

    return round(adjusted_hold, 2), target_home_line, target_away_line
