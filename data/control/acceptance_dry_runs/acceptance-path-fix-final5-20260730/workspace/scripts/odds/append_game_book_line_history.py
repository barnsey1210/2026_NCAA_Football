...
def main():
 ...
 # Ingestion/snapshot time is metadata, not market-state identity.  Some input
 # files do not carry a provider update time, so including the generated `now`
 # value would append the same quote on every invocation.
 identity=["source","game_key","book","market","line","price"]
 for c in identity:
 ...
 def key(frame):
  ...
  for c in identity:
   if c in ("line","price"):
    # CSV round-trips commonly turn -110 into -110.0; normalize numeric
    # representation so that serialization does not manufacture movement.
    v=pd.to_numeric(frame[c],errors="coerce").map(lambda x:"" if pd.isna(x) else f"{float(x):.10g}")
   else:
    v=frame[c].fillna("").astype(str)
   parts.append(v)
  result=parts[0]
