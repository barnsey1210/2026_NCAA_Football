...
def normalize_action_spread_pair(home_line, away_line):
    """
    A valid two-sided spread must have opposite signs:
      home_line = -away_line

    Some Action rows occasionally emit both home and away with the same sign.
    When both sides are same-sign, trust the away row as side-perspective and
    convert the home-perspective line to -away_line.
    """
    try:
    ...
    if (h > 0 and a > 0) or (h < 0 and a < 0):
        return -a, a

    if abs(abs(h) - abs(a)) > 0.001:
        return h, -h
