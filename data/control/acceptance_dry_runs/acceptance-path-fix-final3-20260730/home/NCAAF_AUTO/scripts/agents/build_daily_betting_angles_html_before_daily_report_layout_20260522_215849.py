...
if df.empty:
    ...
    <section>
      ...
      <p class="muted">Win totals and conference futures moves from the latest report, sorted by betting relevance and movement size.</p>
    </section>
    """)

    for cat, group in df.groupby("category", sort=False):
        limit = 15 if cat == "Market move" else (8 if cat == "Arbitrage" else 5)
...
<div class="summary">Daily market moves are listed first so you can quickly see where win totals and conference futures are shifting. Arbitrage and middle opportunities are included after the move section when available.</div>
{sections}
</div>
</body>
</html>
