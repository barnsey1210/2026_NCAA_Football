...
def main():
    ...
    daily_issues={
      "newest-book/FanDuel selection":"CORRECTED: shared display artifact uses DraftKings then Bovada only",
      "cross-book movement comparisons":"CORRECTED: paired same-book artifact is the only SGO accepted/history input",
      "exact-name matching":"CORRECTED PATH: controller canonical mapping and aliases are shared; unmatched events are reported",
      "silent unmatched games":"CORRECTED: coverage artifact reports unmatched/ambiguous/missing games",
      "canonical game_id mapping":"CORRECTED: required in every quote/display/history row",
      "Week 0 and Week 1 merging":"CORRECTED: canonical site week is used, provider week is provenance only",
      "pagination and partial coverage":"ACQUISITION STILL PARTIAL; acceptance gate now blocks any remaining cursor",
      "neutral-site loss":"CORRECTED: canonical neutral-site value retained",
      "unavailable-market fallback":"CORRECTED: unavailable/suspended quotes excluded",
      "stale-market fallback":"CORRECTED: explicit quote-age/stale gate",
      "independently paired total sides":"CORRECTED: same-book equal-line pair required",
      "independently paired moneyline sides":"CORRECTED: same-book two-side pair required",
      "audit observation deduplication":"PREVIEW-ONLY (corrected quote ledger uses stable quote identity)",
      "canonical history deduplication":"CORRECTED: display and per-book state identities exclude ingestion timestamp; identical replay appends zero",
    }
    report={
      ...
      "known_acceptance_path_gaps":["Archived response coverage is partial; complete bounded acquisition is required before a real acceptance preview"],
      "classification":"NEEDS MORE ACCEPTANCE-PATH FIXES" if failures or any(idempotent.values()) else "READY FOR COMPLETE-COVERAGE ACCEPTANCE PREVIEW",
