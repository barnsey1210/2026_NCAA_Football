...
(function(){
  ...
  function sourceRows(){
    const defaultRows = __DEFAULT_ROWS__;
  ...
  function productionText(rows){
    ...
    if (!active.length) return 'Active 2026 ratings only — __MODEL_SUMMARY__. Reference/stale sources are shown for context but excluded from production projections/sims.';
...
for path in TARGETS:
    ...
    active = {x["key"]: x for x in model["sources"]}
    labels = {"spplus": "SP+", "fpi": "FPI", "teamrankings": "TeamRankings", "kford": "KFord", "bradpowers": "Brad Powers"}
    default_rows = [{"name": labels[k], "active": k in active, "weight": active.get(k, {}).get("weight", 0), "teams": 138 if k in active else 136} for k in labels]
    block = BLOCK.replace("__DEFAULT_ROWS__", json.dumps(default_rows, separators=(",", ":"))).replace("__MODEL_SUMMARY__", model_summary())
