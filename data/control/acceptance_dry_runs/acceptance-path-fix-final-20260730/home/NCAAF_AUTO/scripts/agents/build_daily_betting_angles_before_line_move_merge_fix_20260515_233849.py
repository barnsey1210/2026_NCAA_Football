...
def main():
    ...
    if not moves.empty:
        ...
        # Combine opposite-side price moves for the same team/book/date/line into one report row.
        ...
        for key, g in win_moves.groupby(group_cols, dropna=False):
            ...
            if len(over) and len(under):
                o = over.iloc[0]
                u = under.iloc[0]
                max_imp = max(float(o.get("abs_imp") or 0), float(u.get("abs_imp") or 0))
                title = f"{team} {total} win total moved at {book}"
                reason = (
                    f"Over {total}: {o.get('previous')} → {o.get('latest')}; "
                    f"Under {total}: {u.get('previous')} → {u.get('latest')} on {move_date}."
                )
                combined.append({
                    "team": team,
                    "title": title,
                ...
                combined.append({
                    "team": team,
                    "title": f"{team} {r.get('field')} moved",
                    "score": float(r.get("abs_imp") or 0),
                    "reason": f"{book} {r.get('field')}: {r.get('previous')} → {r.get('latest')} on {move_date}.",
                    "field": r.get("field"),
                    "move_date": move_date,
                })
        ...
        for _, r in non_win.iterrows():
            combined.append({
                "team": canon_team(r.get("team", "")),
                "title": f"{canon_team(r.get('team', ''))} {r.get('field')} moved",
                "score": float(r.get("abs_imp") or 0),
