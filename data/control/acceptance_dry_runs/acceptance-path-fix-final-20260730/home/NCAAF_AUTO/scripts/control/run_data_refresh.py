...
Version one deliberately enables status and dry-run operations only. External
provider calls and publication require both configuration enablement and an
explicit command-line confirmation. No credentials are read into status files.
"""
...
def main() -> int:
    ...
    if explicit_providers:
    ...
    elif args.mode in {"odds", "pregame", "full"}:
        ...
        resolved_providers = []
    # The only executable provider path currently authorized is the separately
    # reviewed SGO games-only preview. Normal mode previews may describe the
    # complete canonical pipeline without being narrowed to that experiment.
    if args.mode == "odds" and args.execute:
        ...
        if args.scope != "games" or args.week != "current" or requested_markets != ["spread","total","moneyline"] or requested_providers != ["sports_game_odds"]:
            raise SystemExit("Stage 2 odds preview requires scope=games, week=current, markets=spread,total,moneyline, providers=sports_game_odds")
        if args.confirm_publish:
            raise SystemExit("Stage 2 odds preview forbids publication confirmation")
    ...
    try:
        ...
        requested = resolved_providers if args.mode in {"odds","pregame","full"} else []
        blocked = []
        ...
        if blocked:
            ...
            if args.dry_run:
                ...
                if not os.environ.get("SGO_API_KEY"):
                    ...
                    state.setdefault("sports_game_odds", {}).update({"last_attempt_epoch":time.time(),"last_attempt_at":now(),"last_run_id":run_id,"status":"ATTEMPTING_PREVIEW"})
                    atomic_json(STATE,state)
                    preview=capture(run_id)
                    state["sports_game_odds"].update({"status":"PREVIEW_COMPLETED","last_success_at":now(),"actual_cost":preview.get("actual_api_cost")})
                    atomic_json(STATE,state)
                    run["providers_called"]=["sports_game_odds"]
                    run["api"]["calls_consumed"]=preview["external_calls"]
                    run["api"]["estimated_cost"]=preview["estimated_api_cost"]
                    run["api"]["actual_cost"]=preview.get("actual_api_cost")
                    run["preview"]=preview
                    run["stages"].append({"name":"sports_game_odds_preview","status":"PASSED","timestamp":now(),"staging_manifest":f"data/control/staging/{run_id}/manifest.json"})
                ...
                run["stages"].append({"name":"canonical_publish_check","status":"PASSED" if check["returncode"]==0 else "FAILED", **check})
                if check["returncode"]: run["status"]="FAILED"; run["errors"].append("canonical publish check failed")
                elif args.confirm_publish and cfg.get("automatic_publication_enabled",False):
                    pub=shell(["bash","scripts/publish/publish_site.sh","--push"]); run["publication"]={"status":"COMPLETED" if pub["returncode"]==0 else "FAILED",**pub}; run["status"]="COMPLETED" if pub["returncode"]==0 else "FAILED"
                else: run["status"]="COMPLETED_WITH_WARNINGS"; run["warnings"].append("validation passed; publication not enabled/confirmed")
            else:
                run["status"]="BLOCKED_BY_CONFIGURATION"; run["errors"].append("live adapters intentionally disabled in version one")
