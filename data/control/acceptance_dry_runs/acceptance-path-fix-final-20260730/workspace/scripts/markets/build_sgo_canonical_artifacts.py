...
def main():
 ...
 selected=manifest.get("resolved_canonical_week");expected_ids={str(x["game"]["game_id"]) for x in canonical.get("games",[]) if int(x["game"].get("week",-1))==int(selected) and all((x.get("teams",{}).get(side)or{}).get("overall_rank") is not None for side in ("away","home"))};mapped_ids={q["canonical_game_id"] for q in quotes};missing_ids=sorted(expected_ids-mapped_ids);coverage={"schema_version":"canonical-market-coverage-v1","provider":"sports_game_odds","scope":"FBS-vs-FBS canonical site week","selected_week":selected,"expected_canonical_games":len(expected_ids),"expected_canonical_game_ids":sorted(expected_ids),"mapped_canonical_games":len(expected_ids&mapped_ids),"mapped_canonical_game_ids":sorted(expected_ids&mapped_ids),"missing_canonical_games":len(missing_ids),"missing_canonical_game_ids":missing_ids,"raw_pages_fetched":manifest.get("page_count",1),"next_cursor_remaining":manifest.get("next_cursor_present",False),"coverage_status":manifest.get("coverage_status"),"request_unit_estimate":0,"request_unit_actual":0,"acceptance_eligibility":global_ok and not missing_ids,"acceptance_block_reason":"" if global_ok and not missing_ids else "partial coverage, remaining cursor, or incomplete canonical-week mapping","unmatched_events":manifest.get("events_unmatched",0),"ambiguous_events":manifest.get("events_ambiguous",0),"accepted_quote_rows":len(quotes),"display_rows":len(display),"payload_hash":payload_hash}
