...
RAW = Path("data/markets/sgo/sgo_ncaaf_events_raw.json")
...
def best_book_by_value(entries, value_key, prefer):
    ...
    if prefer in ("home", "away"):
        best_value = max(c["value"] for c in candidates)
        tied = [c for c in candidates if c["value"] == best_value]

        return sorted(
            tied,
            key=lambda c: (
                c["odds"] if c["odds"] is not None else -99999,
    ...
    if prefer == "over":
        best_value = min(c["value"] for c in candidates)
        tied = [c for c in candidates if c["value"] == best_value]
