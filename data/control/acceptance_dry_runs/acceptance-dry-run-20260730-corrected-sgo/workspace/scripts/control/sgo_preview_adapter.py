...
def process_payload(payload, pulled_at, run_id, root=ROOT, stale_hours=24.0, requested_week="current", replay_source=None):
    ...
    staged_pairs = [(event, game) for event, game in mapped if int(game.get("week")) == resolved_week]
    ...
    manifest = {
        ...
        "replay_source_run_id": replay_source, "external_calls": 0 if replay_source else 1,
        ...
        "page_count": 1, "coverage_status": "PARTIAL" if payload.get("nextCursor") else "COMPLETE",
        "archive_page_coverage": "partial_first_page" if payload.get("nextCursor") else "complete_page",
        "next_cursor_present": bool(payload.get("nextCursor")),
        "pagination_design": "Future live preview must follow nextCursor under an explicit request budget; no pagination was executed in this replay.",
        ...
        "stale_threshold_rationale": "24 hours is a proposed conservative preview threshold; it is explicit/configurable and is not an acceptance policy.",
...
- COVERAGE: {manifest['coverage_status']} (one archived page; nextCursor={manifest['next_cursor_present']})
- ACCEPTANCE: SKIPPED
- PUBLICATION: SKIPPED
...
def capture(run_id, fixture=None, stale_hours=24.0):
    pulled_at = utcnow()
    ...
    if fixture:
        ...
        request = urllib.request.Request(url, headers={"Accept": "application/json", "X-API-Key": key, "User-Agent": "NCAAF-Control/2.0"})
    ...
    atomic(raw_dir / "meta.json", json.dumps({"run_id": run_id, "provider": "sports_game_odds", "pulled_at": pulled_at, "endpoint": ENDPOINT, "query": params, "external_calls": external_calls, "accepted_state_modified": False, "publication": "SKIPPED"}, indent=2, sort_keys=True) + "\n")
    # Fixtures are provider-free and must never be counted as a live request.
    return process_payload(payload, pulled_at, run_id, ROOT, stale_hours, "current", "fixture" if fixture else None)
