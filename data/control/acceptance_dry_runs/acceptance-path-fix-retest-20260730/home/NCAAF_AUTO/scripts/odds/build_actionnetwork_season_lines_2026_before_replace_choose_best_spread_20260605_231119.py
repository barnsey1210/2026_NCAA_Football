...
def is_invalid_same_book_spread_pair(home_line, away_line):
    """
    Conservative validation only.

    Same sportsbook, same market, two-sided standard spread should have opposite signs:
      home_line = -away_line

    If the same book has both sides with the same nonzero sign, do NOT flip/invent a line.
    Exclude that book's spread pair from spread selection.

    Cross-book best lines can still produce both teams as underdogs; that is valid and should
    be handled later as middle/arb opportunity.
    """
    try:
