...
function dashboard(g){{const se=spreadEdge(g), te=totalEdge(g), mv=moveText(g), ce=coachEdge(g), ie=injuryEdge(g); const proj=favoriteFromProjection(g); return `<div class="dashboard">
  <div class="edge-card"><div class="label">Spread Model vs Market</div><div class="edge-main"><span class="pill ${{edgeClass(se.pts)}}">${{esc(se.team)}} ${{se.team==='No edge'?'':signed(se.pts,1)}}</span></div><div class="edge-note">${{esc(se.note)}}</div></div>
  <div class="edge-card"><div class="label">Total Model vs Market</div><div class="edge-main"><span class="pill ${{edgeClass(te.pts)}}">${{esc(te.side)}} ${{te.side==='No edge'||te.side==='No total'?'':signed(te.pts,1)}}</span></div><div class="edge-note">${{esc(te.note)}}</div></div>
