...
- At this checkpoint, 43 of the 61 registered primary script paths are absent from the authoritative source repository. Of those, 41 exist at their primary path in the operational runtime; the two projection builders resolve through their retained root-level runtime fallbacks. The consolidation does not copy, replace, or remove them. This source-coverage gap is the largest remaining architecture risk and requires separate provenance-based migration rather than broad synchronization.
