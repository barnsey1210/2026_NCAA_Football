...
| Odds | `scripts/site/build_odds_screen_v2.py`, `build_odds_futures_v2.py` | `odds.html` | `#oddsStatus` | `odds_screen_v2.json`, `odds_futures_v2.json`; provider/page timestamps | games, spreads, totals, moneylines, books, unavailable | GREEN below 24h with clean current-board validation; YELLOW from 24h through 48h, or for isolated provider-identity inventory and excluded malformed input when usable valid quotes remain. RED beyond 48h, when odds risk attachment to the wrong canonical game, malformed quotes reach display, malformed input causes critical coverage loss, critical artifacts fail, or stale data is presented as current. |
